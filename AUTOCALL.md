あなたは熟練のFull-Stackエンジニアであり、Cloudflare Workersのスペシャリストです。
これから提示する仕様とソースコードを元に、Cloudflare Workersを用いた「Zoom Phone自動架電Webアプリケーション」の構築、環境設定、およびデプロイまでを【完全に自動】で行ってください。

### 1. 構築するシステムの仕様
- バックエンドAPIとHTMLフロントエンド（画面）が1つのファイルに統合されたCloudflare Workers（TypeScript）アプリです。
- 主な機能：
  1) `GET /` : 管理ダッシュボード画面（Tailwind CSS仕様）を表示
  2) `POST /upload` : 任意のCSVから日本の電話番号を抽出し、1件目に発信
  3) `POST /toggle-status` : 自動架電の一時停止/再開のステータス制御
  4) `POST /webhook` : Zoom Phoneの切電検知（phone.call_ended）と次の番号への自動発信
  5) `GET /results` : 架電結果CSVのダウンロード窓口
- データの保存先として、CloudflareのKV（Key-Valueストア）を使用します（バインド名は `PHONE_STORE`）。

### 2. あなた（AIエージェント）に実行してほしい手順
以下のステップを、あなたのターミナル実行機能（CLI操作）やファイル作成機能を使って、順番にエラーなく実行してください。

#### ステップ A: プロジェクトの初期化
1. ターミナルで `npm create cloudflare@latest zoom-dialer -- --type=javascript` を実行するか、または同等の構成（TypeScript対応）で `zoom-dialer` ディレクトリを作成してください。全てデフォルトの最小構成（ハローワールド）で構いません。
2. 作成した `zoom-dialer` ディレクトリに移動（cd）してください。

#### ステップ B: KV（データベース）の作成
1. ターミナルで `npx wrangler kv:namespace create PHONE_STORE` を実行してください。
2. 実行結果としてターミナルに出力される `[[kv_namespaces]]` の設定（idの文字列が含まれるもの）をキャッチしてください。

#### ステップ C: 設定ファイル（wrangler.toml）の作成・書き換え
プロジェクトのルートにある `wrangler.toml` を以下のように作成・書き換えてください。ステップBで取得したKVのIDをセットしてください。

[期待するwrangler.tomlの構造]
------------
name = "zoom-dialer"
main = "src/index.ts"
...
[[kv_namespaces]]
binding = "PHONE_STORE"
id = "【ステップBで出力された実際のIDをここに自動挿入】"
------------

#### ステップ D: メインコードの配置
`src/index.ts`（既存のファイルがある場合は上書き）に、以下の【ソースコード】を完全にそのまま書き込んでください。
...
#### ステップ E: プレデプロイテストと仕上げ
1. `npm run` や `npx wrangler` で型エラーやビルドエラーが出ないかチェックしてください。
2. すべてのファイルの作成と設定が完了したら、最後に「Cloudflareへのデプロイコマンド（`npx wrangler deploy`）」の手前まで、あるいはあなたの判断でデプロイを実行可能な状態に仕上げて、私（ユーザー）に報告してください。

上記の指示に従い、まずはプロジェクトの作成（ステップA）から開始し、完了するごとに進捗を教えてください。