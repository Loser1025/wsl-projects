...
async function downloadFromDrive(driveUrl) {
  return new Promise((resolve, reject) => {
    ...
    if (!fileId) {
      reject(new Error('無効なGoogle Drive URLです'));
      return;
    }
...
// Google Driveから動画をダウンロードして分析する関数
async function downloadAndAnalyzeFromDrive(driveUrl) {
  try {
    // Google Driveから動画をダウンロード
    const file = await downloadFromDrive(driveUrl);
    ...
    console.error('Google Driveからの分析中にエラーが発生しました:', error);
    return {
      error: true,
      message: 'Google Driveからの分析に失敗しました: ' + error.message,
