"""
クレジット(Square)の初回支払日が空の行を契約日で補完する
"""
...
WHERE contract_id IN ({contract_ids})
  AND payment_method_slug = 'cc_square'
""", token)
square_ids = {r["contract_id"] for r in bq_rows}
print(f"うち cc_square: {len(square_ids)}件")

# 書き込み対象を絞る
batch_data = []
for t in targets:
    if t["contract_id"] in square_ids:
        batch_data.append({
            "range": f"J{t['row']}",
            "values": [[t["contracted"]]]
        })
...
print(f"\n完了：cc_square {len(batch_data)}行の初回支払日に契約日を設定しました")
