"""
ローン会社別ルールで初回支払日の推定値を入力する

ルール:
  ポケットカード : 契約日に関わらず → 翌々月1日
  アプラス       : 1〜5日  → 翌月27日 / 6〜末日 → 翌々月27日
  日本プラム     : 1〜25日 → 翌々月5日 / 26〜末日 → 翌々々月5日
  ライフティ     : 1〜20日 → 翌月27日 / 21〜末日 → 翌々月27日
  CBS            : 契約日に関わらず → 翌々月27日
"""
import sys, io, requests, subprocess, calendar
from datetime import datetime
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8")
import gspread
from google.oauth2.service_account import Credentials

SA_FILE = r"C:\Users\Loser\Desktop\-\tamalabo\automation-visitor-shindan\ageless-impulse-488713-m6-03014b3cddad.json"
SHEET_ID = "1NQU2SGVykYL3n35NgzL78R0fszK0vt5yacNSV151wYI"
SHEET_NAME = "2026年5月16日時点未解約データ"
SCOPES = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
GCLOUD_PATH = r"C:\Users\Loser\AppData\Local\Google\Cloud SDK\google-cloud-sdk\bin\gcloud.ps1"
BQ_PROJECT = "stream-443709"

def get_token():
    r = subprocess.run(["powershell", "-NoProfile", "-Command", f"& '{GCLOUD_PATH}' auth print-access-token"],
        capture_output=True, text=True, encoding="utf-8")
    return r.stdout.strip()

def bq_query(sql, token):
    url = f"https://bigquery.googleapis.com/bigquery/v2/projects/{BQ_PROJECT}/queries"
    resp = requests.post(url,
        headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
        json={"query": sql, "useLegacySql": False, "timeoutMs": 30000, "maxResults": 5000})
    data = resp.json()
    if not resp.ok:
        raise RuntimeError(data)
    fields = [f["name"] for f in data.get("schema", {}).get("fields", [])]
    return [{fields[i]: r["f"][i].get("v") for i in range(len(fields))} for r in data.get("rows", [])]

def add_months(dt, months):
    """日付にmonths月加算して返す"""
    month = dt.month + months
    year  = dt.year + (month - 1) // 12
    month = (month - 1) % 12 + 1
    day   = min(dt.day, calendar.monthrange(year, month)[1])
    return datetime(year, month, day)

def calc_first_pay(contracted_str, slug):
    """ローン会社ルールで初回支払日を計算"""
    try:
        d = datetime.strptime(contracted_str.strip(), "%Y/%m/%d")
    except Exception:
        return None

    day = d.day

    if slug == "ml_pocketcard":
        # 翌々月1日
        base = add_months(d, 2)
        return datetime(base.year, base.month, 1)

    elif slug == "ml_aplus":
        # 1〜5日 → 翌月27日、6〜末日 → 翌々月27日
        months = 1 if day <= 5 else 2
        base = add_months(d, months)
        pay_day = min(27, calendar.monthrange(base.year, base.month)[1])
        return datetime(base.year, base.month, pay_day)

    elif slug == "ml_jplum":
        # 1〜25日 → 翌々月5日、26〜末日 → 翌々々月5日
        months = 2 if day <= 25 else 3
        base = add_months(d, months)
        return datetime(base.year, base.month, 5)

    elif slug == "ml_ryfety":
        # 1〜20日 → 翌月27日、21〜末日 → 翌々月27日
        months = 1 if day <= 20 else 2
        base = add_months(d, months)
        pay_day = min(27, calendar.monthrange(base.year, base.month)[1])
        return datetime(base.year, base.month, pay_day)

    elif slug == "ml_cbsfs":
        # 翌々月27日
        base = add_months(d, 2)
        pay_day = min(27, calendar.monthrange(base.year, base.month)[1])
        return datetime(base.year, base.month, pay_day)

    return None

def fmt(dt):
    return dt.strftime("%Y/%m/%d") if dt else ""

def add_one_month(dt):
    return add_months(dt, 1)

# スプレッドシートから対象行を取得
creds = Credentials.from_service_account_file(SA_FILE, scopes=SCOPES)
ws = gspread.authorize(creds).open_by_key(SHEET_ID).worksheet(SHEET_NAME)
all_rows = ws.get_all_values()

targets = []
for i, row in enumerate(all_rows[1:], start=2):
    contract_id = row[5].strip() if len(row) > 5 else ""
    first_pay   = row[9].strip() if len(row) > 9 else ""
    contracted  = row[8].strip() if len(row) > 8 else ""
    if contract_id and not first_pay and contracted:
        targets.append({"row": i, "contract_id": contract_id, "contracted": contracted})

print(f"初回支払日空・契約日あり: {len(targets)}行")

# BQでローン会社のslugを取得
token = get_token()
contract_ids = ", ".join(set(t["contract_id"] for t in targets))
bq_rows = bq_query(f"""
SELECT CAST(contract_id AS STRING) AS contract_id, payment_method_slug
FROM `stream-443709.stream.contract_payment_method`
WHERE contract_id IN ({contract_ids})
  AND payment_method_slug IN ('ml_pocketcard','ml_aplus','ml_jplum','ml_ryfety','ml_cbsfs')
""", token)
slug_map = {r["contract_id"]: r["payment_method_slug"] for r in bq_rows}

LOAN_NAMES = {
    "ml_pocketcard": "ポケットカード",
    "ml_aplus":      "アプラス",
    "ml_jplum":      "日本プラム",
    "ml_ryfety":     "ライフティ",
    "ml_cbsfs":      "CBS",
}

# 計算して書き込みデータを作成
batch_data = []
slug_counts = {s: 0 for s in LOAN_NAMES}

for t in targets:
    slug = slug_map.get(t["contract_id"])
    if not slug:
        continue
    first_dt = calc_first_pay(t["contracted"], slug)
    if not first_dt:
        continue
    second_dt = add_one_month(first_dt)
    batch_data.append({
        "range": f"J{t['row']}:K{t['row']}",
        "values": [[fmt(first_dt), fmt(second_dt)]]
    })
    slug_counts[slug] += 1
    # サンプル表示（各社先頭3件）
    if slug_counts[slug] <= 3:
        print(f"  [{LOAN_NAMES[slug]}] row={t['row']}: 契約日={t['contracted']} → 初回={fmt(first_dt)}, 2回目={fmt(second_dt)}")

print(f"\n書き込み対象: {len(batch_data)}行")
for slug, cnt in slug_counts.items():
    if cnt:
        print(f"  {LOAN_NAMES[slug]}: {cnt}件")

# スプレッドシートに書き込み
for i in range(0, len(batch_data), 200):
    ws.batch_update(batch_data[i:i+200], value_input_option="USER_ENTERED")
    print(f"  {min(i+200, len(batch_data))}/{len(batch_data)}件 完了")

print(f"\n=== 完了 ===")
print(f"  {len(batch_data)}行の初回支払日・2回目支払日を入力しました（推定値）")
