...
4. 【検証】Pipeline-First系の読み取りツール（grep_codebase, search_in_file,
   ...
   本当にユーザーの要求を満たしているか確認する。既存のテスト/ビルド/lintコマンドが
   あれば delegate_to_team の verify_cmd に指定すること。Worker実行後に同じ作業
   ディレクトリでそのコマンドが自動実行され、終了コード・出力がSupervisorの判定材料
   になる（あなた自身はテストを実行できないため、これが構造的な検証手段になる）。
